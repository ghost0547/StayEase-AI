...
function SearchFilterBar({
  ...
  return (
    <div className="w-full bg-white/75 dark:bg-slate-900/75 border border-slate-200/80 dark:border-slate-800/80 backdrop-blur-2xl rounded-3xl p-5 sm:p-7 shadow-2xl shadow-cyan-500/5 hover:border-cyan-500/30 mb-10 space-y-6 transition-all duration-300">
      {/* Top Controls Row: Dominant Search Bar (7 cols) + Price Dropdown (2.5 cols) + Sort Dropdown (2.5 cols) */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        {/* 1. Search Bar - Dominant Control (7/12 cols on desktop) */}
        ...
        <div className="md:col-span-6 lg:col-span-2.5 relative">
          <div className="relative flex items-center">
            ...
            <HiChevronDown className="w-4 h-4 text-slate-400 absolute right-3 pointer-events-none" />
          </div>
        </div>

        {/* 3. Sort Dropdown (2.5/12 cols on desktop) */}
        <div className="md:col-span-6 lg:col-span-2.5 relative">
          <div className="relative flex items-center">
            <HiArrowsUpDown className="w-4 h-4 text-teal-600 dark:text-teal-400 absolute left-3.5 pointer-events-none" />
            <select
              value={sortBy}
        ...
        {CATEGORY_CHIPS.map((chip) => {
          ...
          return (
            <motion.button
              ...
              className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold shrink-0 transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-cyan-500/30 ${
                isActive
            ...
            >
              <Icon className={`w-4 h-4 ${isActive ? "text-slate-950" : "text-cyan-500 dark:text-cyan-400"}`} />
      ...
      <div className="pt-3.5 border-t border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-sm font-semibold text-slate-700 dark:text-slate-300">
          <span className="text-cyan-600 dark:text-cyan-400 font-extrabold text-base sm:text-lg">
            {filteredCount}
          </span>
